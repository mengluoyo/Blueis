...
namespace blueis
{
    ...
    public:
        ...
        // === 数据库管理 ===
        void create_database(const std::string &name);      // 创建数据库
        void drop_database(const std::string &name);        // 删除数据库
        bool use_database(const std::string &name);         // 使用数据库
        std::vector<std::string> show_databases() const;    // 查询数据库
         ...
         * @param pk_col  主键字段名
         */
        bool insert_row(const std::string &table, const DataRow &row, const std::string &pk_col);
    ...
    private:
        ...
        struct DataBase
        {
            std::unordered_map<std::string, TableMeta> m_tables; // SQL表
            // key: "table:pkValue" → DataRow
            std::unordered_map<std::string, std::unordered_map<std::string, DataRow>> m_rows;
        };
        std::unordered_map<std::string, DataBase> m_databases; // SQL库，调用访问
    ...
    public:
        // SQL
        std::string m_database_name = "";                 // 当前选择的数据库
    };
